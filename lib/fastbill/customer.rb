module Fastbill
  class Customer < Fastbill::Model
    ...
    class << self
      def find_by_id(id)
        response = Fastbill::Request.post({"SERVICE" => "customer.get", "FILTER" => {"CUSTOMER_ID" => id.to_i }}.to_json)
        return nil if response[:CUSTOMERS].empty?
        new(response[:CUSTOMERS][0])
      end

      def find_by_customer_number(number)
        response = Fastbill::Request.post({"SERVICE" => "customer.get", "FILTER" => {"CUSTOMER_NUMBER" => number }}.to_json)
        return nil if response[:CUSTOMERS].empty?
        new(response[:CUSTOMERS][0])
      end
      
      def find_by_city(city)
        response = Fastbill::Request.post({"SERVICE" => "customer.get", "FILTER" => {"CITY" => city }}.to_json)
      ...
      def find_by_country_code(country_code)
        response = Fastbill::Request.post({"SERVICE" => "customer.get", "FILTER" => {"COUNTRY_CODE" => country_code.upcase}}.to_json)
        response[:CUSTOMERS].collect { |customer| new(customer) }
      end

      def search(term)
        response = Fastbill::Request.post({"SERVICE" => "customer.get", "FILTER" => {"TERM" => term}}.to_json)
        response[:CUSTOMERS].collect { |customer| new(customer) }
      end
    end
  end
