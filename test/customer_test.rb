...
describe Fastbill::Customer do
  ...
  describe "find methods" do
    ...
    describe "customers" do
      ...
      it "#all" do
        customers = Fastbill::Customer.all
      ...
      it "#find_by_country" do
        expected_ids = JSON.parse(fixture_file("customers.json"))["RESPONSE"]["CUSTOMERS"].map { |c| c["CUSTOMER_ID"] }
        Fastbill::Customer.find_by_country("de").map(&:id).must_equal expected_ids
      end

      it "#find_by_customer_number" do
        expected_id = JSON.parse(fixture_file("customers.json"))["RESPONSE"]["CUSTOMERS"].first['CUSTOMER_ID']
        Fastbill::Customer.find_by_customer_number("2").id.must_equal expected_id
      end
    end
    ...
    describe "customer" do
      ...
      it 'should have a model name' do
        Fastbill::Customer.model_name.must_equal 'customer'
      end

      it 'should have a model name' do
        Fastbill::Customer.model_name_plural.must_equal 'customers'
      end
      
      it "#find_by_id" do
        expected_customer_id = JSON.parse(fixture_file("customer.json"))["RESPONSE"]["CUSTOMERS"][0]["CUSTOMER_ID"]
        Fastbill::Customer.find_by_id(expected_customer_id).id.must_equal expected_customer_id
      end

      it "#search" do
        expected_id = JSON.parse(fixture_file("customer.json"))["RESPONSE"]["CUSTOMERS"][0]["CUSTOMER_ID"]
        Fastbill::Customer.search("foo").first.id.must_equal expected_id
      end
    end
  ...
  describe "create, delete, update" do
    it "#create" do
      ...
      customer = Fastbill::Customer.create(last_name: "foo", first_name: "bar", city: "dummy", customer_type: "business", organization: "foobar")
    ...
    it '#update' do
      ...
      customer = Fastbill::Customer.new
